...
public record Bounds(boolean relative, Optional<Integer> x, Optional<Integer> y, Optional<Integer> width, Optional<Integer> height) {
    public static Codec<Bounds> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.BOOL.optionalFieldOf("relative").forGetter(bounds -> Optional.of(bounds.relative)),
    ...
    ).apply(instance, (relative, x, y, width, height) -> new Bounds(relative.orElse(false), x, y, width, height)));
